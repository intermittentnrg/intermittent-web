...
export type Resolution = (typeof RESOLUTION_BUCKETS)[number]["label"];
...
export function parseDateRange(fromValue: string | undefined, toValue: string | undefined, now = new Date()): ParsedDateRange {
  return {
...
export function parseAppDate(value: string | undefined, options: { end?: boolean; now?: Date } = {}): Date {
...
export function resolutionToSeconds(resolution: string | undefined, fallback: Resolution = "15m") {
  return RESOLUTION_SECONDS[resolution as Resolution] || RESOLUTION_SECONDS[fallback];
}

export function calculateResolution(from: Date, to: Date, widthValue?: string | number, minResolution: Resolution = "5m"): Resolution {
...
function previousPeriod(unit: string, now: Date, end: boolean) {
  if (unit === "week") {
