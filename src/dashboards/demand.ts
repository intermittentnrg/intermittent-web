...
export async function demand(
  ...
  const rows = await querySmall<BasicRow>(demandSql, [
    `${interval} seconds`,
    ctx.from,
    ctx.to,
    ctx.areaIds,
  ...
  return sendChart(
    reply,
    buildPowerLineSeries(rows, true),
    "Demand",
...
export async function demandMinMax(
  ...
  const rows = await querySmall<Row>(demandMinMaxSql, [
    `${interval} seconds`,
    ctx.from,
    ctx.to,
    ctx.areaIds,
...
export async function demandYoy(
  ...
  const rows = await querySmall<Row>(demandYoySql, [
    `${interval} seconds`,
    finish,
    ctx.areaIds,
    ctx.timezone,
